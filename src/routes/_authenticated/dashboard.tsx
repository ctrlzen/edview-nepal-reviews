        ...
        {reviews.map((r) => {
          ...
          return (
            ...
            >
              ...
              <p
                className={`mt-1.5 text-sm text-muted-foreground ${expanded ? "" : "line-clamp-2"}`}
              >
                {r.body}
              </p>
    ...
    <section className="animate-fade-in-up">
      ...
      <div className="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-3 stagger">
        {items.map((a) => {
          ...
          return (
            <div
              key={a.collegeSlug + a.program}
              className="flex flex-col rounded-2xl border border-border bg-card p-5 shadow-soft"
            >
              <div className="flex items-start justify-between">
                <Link
                  to="/colleges/$slug"
                  params={{ slug: a.collegeSlug }}
                ...
                <span
                  className={`rounded-full px-2.5 py-1 text-[10px] font-semibold ${urgent ? "bg-destructive/10 text-destructive" : "bg-brand-soft text-brand"}`}
                >
                  {days}d left
                </span>
              </div>
    ...
    <section className="animate-fade-in-up">
      ...
      {isLoading ? (
        ...
        <div className="mt-5 rounded-2xl border border-dashed border-border bg-card p-8 text-center text-sm text-muted-foreground">
          ...
          <Link to="/colleges" className="text-brand hover:underline">
            Browse colleges
          </Link>
        </div>
      ) : (
        <div className="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-4 stagger">
          {items.map((c) => {
            const rs = SEED_REVIEWS.filter((r) => r.collegeSlug === c.slug);
            const avg = rs.length
              ? rs.reduce((s, r) => s + avgOverall(r.ratings), 0) / rs.length
              : 0;
            return (
              <div
                key={c.slug}
                className="group relative flex flex-col rounded-2xl border border-border bg-card p-5 shadow-soft"
              >
                <button
